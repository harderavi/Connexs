...
const Button = ({children , variant='primary', clickHandle, disabled=false, className}:ButtonProps) => {
  ...
  return (
    <button className={`${BtnVariant} ${disable} py-3 rounded-lg    flex justify-center items-center gap-x-2 ${className}  `} disabled={disabled} onClick={clickHandle}>
        {children}</button>
  )
}
